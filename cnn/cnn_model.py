...
class CNN(Model):
    ...
    def generate_model_small(self, input_pl=None, bn_decay=None, reuse=None):
        input_pl = self.input_pl if input_pl is None else input_pl

        input_channels = input_pl.get_shape()[-1].value
        batch_size = input_pl.get_shape()[0].value

        conv1 = nn_layers.conv2d(input_pl, input_channels, 64, (3, 3), 'conv_1', is_training=self.is_training, reuse=reuse)
        conv2 = nn_layers.conv2d(conv1, 64, 128, (3, 3), 'conv_2', is_training=self.is_training, reuse=reuse)
        pool1 = nn_layers.max_pool2d(conv2, (3, 3), 'pool1', reuse=reuse)
        conv3 = nn_layers.conv2d(pool1, 128, 256, (3, 3), 'conv_3', is_training=self.is_training, reuse=reuse)
        pool2 = nn_layers.max_pool2d(conv3, (3, 3), 'pool2', reuse=reuse)
        conv4 = nn_layers.conv2d(pool2, 256, 512, (1, 1), 'conv_4', is_training=self.is_training, reuse=reuse)
        # conv6 = nn_layers.conv2d(pool4, 256, 512, (1, 1), 'conv_6', is_training=self.is_training, reuse=reuse)
        # conv7 = nn_layers.conv2d(conv6, 512, 1024, (1, 1), 'conv_7', is_training=self.is_training, reuse=reuse)
