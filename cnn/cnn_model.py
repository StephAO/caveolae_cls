...
class CNN(Model):
    ...
    def get_model(self, input_pl, is_training, bn_decay=None):
        ...
        conv1 = nn_layers.conv2d(input_pl, input_channels, 16, (3, 3), 'conv_1', is_training=is_training)
        conv2 = nn_layers.conv2d(conv1, 16, 32, (3, 3), 'conv_2', is_training=is_training)
        pool1 = nn_layers.max_pool2d(conv2, (3, 3), 'pool1')
        conv3 = nn_layers.conv2d(pool1, 32, 64, (3, 3), 'conv_3', is_training=is_training)
        pool2 = nn_layers.max_pool2d(conv3, (3, 3), 'pool2')
        conv4 = nn_layers.conv2d(pool2, 64, 128, (3, 3), 'conv_4', is_training=is_training)
        ...
        fc1 = nn_layers.fc(convnet, input_channels, 256, 'fc1', is_training=is_training)
        fc2 = nn_layers.fc(fc1, 256, 128, 'fc2', is_training=is_training)
        pred = nn_layers.fc(fc2, 128, 1, 'predicted_y', activation_fn=tf.nn.sigmoid, batch_norm=False)
