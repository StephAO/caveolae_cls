...
class CNN(Model):
    ...
    def get_model(self, input_pl, is_training, bn_decay=None, reuse=None):
        ...
        conv1 = nn_layers.conv2d(pool0, input_channels, 16, (3, 3), 'conv_1', is_training=is_training, reuse=reuse)
        conv2 = nn_layers.conv2d(conv1, 16, 32, (3, 3), 'conv_2', is_training=is_training, reuse=reuse)
        pool1 = nn_layers.max_pool2d(conv2, (3, 3), 'pool1', reuse=reuse)
        conv3 = nn_layers.conv2d(pool1, 32, 64, (3, 3), 'conv_3', is_training=is_training, reuse=reuse)
        pool2 = nn_layers.max_pool2d(conv3, (3, 3), 'pool2', reuse=reuse)
        conv4 = nn_layers.conv2d(pool2, 64, 128, (3, 3), 'conv_4', is_training=is_training, reuse=reuse)
        pool3 = nn_layers.max_pool2d(conv4, (3, 3), 'pool3', reuse=reuse)
        conv5 = nn_layers.conv2d(pool3, 128, 256, (3, 3), 'conv_5', is_training=is_training, reuse=reuse)
        ...
        fc1 = nn_layers.fc(convnet, input_channels, 256, 'fc1', is_training=is_training, reuse=reuse)
        fc2 = nn_layers.fc(fc1, 256, 128, 'fc2', is_training=is_training, reuse=reuse)
        pred = nn_layers.fc(fc2, 128, 1, 'predicted_y', activation_fn=tf.nn.sigmoid, batch_norm=False, reuse=reuse)
