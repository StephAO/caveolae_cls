...
class CNN(Model):
    ...
    def generate_model_small(self, input_pl=None, bn_decay=None, reuse=None):
        with tf.variable_scope(type(self).__name__):
            ...
            conv1 = nn_layers.conv2d(input_pl, input_channels, 64, (3, 3), 'conv1', is_training=self.is_training, reuse=reuse)
            conv2 = nn_layers.conv2d(conv1, 64, 128, (3, 3), 'conv2', is_training=self.is_training, reuse=reuse)
            pool1 = nn_layers.max_pool2d(conv2, (3, 3), 'pool1', reuse=reuse)
