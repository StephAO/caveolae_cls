...
class CAE(Model):
    ...
    def encode(self, input_pl, input_channels):
        ...
        conv5 = nn_layers.conv2d(pool4, 256, 16, (1, 1), 'conv5', is_training=self.is_training)

        self.features = nn_layers.max_pool2d(conv5, (2, 2), 'pool5')

    def decode(self, input_channels):
        # Decoder
        deconv1 = nn_layers.conv2d_transpose(self.features, 256, 128, (3, 3), 'deconv1', is_training=self.is_training)
        deconv2 = nn_layers.conv2d_transpose(deconv1, 128, 64, (3, 3), 'deconv1', is_training=self.is_training)
        deconv3 = nn_layers.conv2d_transpose(deconv2, 64, 32, (3, 3), 'deconv1', is_training=self.is_training)
        self.pred = nn_layers.conv2d_transpose(deconv3, 32, input_channels, (2, 2), 'deconv1',
                                               is_training=self.is_training)
        assert(self.pred.shape)
